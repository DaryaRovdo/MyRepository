var theMessage = function(user, text, userFlag) {
	return {
		text: text,
		user: user,
		userFlag: userFlag,
...
function createAllMessages(allMessages) {
	for(var i = 0; i < allMessages.length; i++) {
		if (allMessages[i].deleted == false) {
			if (allMessages[i].user == document.getElementById("name").innerHTML) {
				allMessages[i].userFlag = true;
			}
...
function onSendButtonClick(){
	if (document.getElementById("name").value == "")
		...
		var message = theMessage(document.getElementById("name").value, document.getElementById("messageText").value, true);
...
function restoreToEdit(continueWith) {
	var url = appState.mainUrl + '?token=' + appState.tokenToEdit;
	get(url, function(responseText) {
		console.assert(responseText != null);
		var response = JSON.parse(responseText);
		appState.tokenToEdit = response.token;
